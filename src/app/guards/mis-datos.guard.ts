...
export const misDatosGuard: CanActivateFn = async (route, state) => {
  ...
  // El guardián de la página de misDatos fuerza la navegación hacia la página
  ...
  if (await authService.isAuthenticated()) {
    // Si el usuario está autenticado lo deja entrar a la página de misDatos
    return true;
  } else {
    // Si el usuario no está autenticado lo devuelve a la página de ingreso
    router.navigate(['/ingreso']);
    // Pero le niega la entrada a la página de misDatos
    return false;    
  }
}
